...
- [`mwc-libp2p-floodsub` CHANGELOG](protocols/floodsub/CHANGELOG.md)
...
- Update `mwc-libp2p-core` and all dependent crates.
...
- Bump minimum `mwc-libp2p-core` patch version.
...
- Update to `mwc-libp2p-core` `0.22.1`.
...
or the provided legacy configuration for `mwc-libp2p-noise` used!

- Bump `mwc-libp2p-noise` dependency to `0.22`. See the `mwc-libp2p-noise`
changelog for details about the `LegacyConfig`.
...
**NOTE**: For a smooth upgrade path from `0.21` to `> 0.22`
on an existing deployment using `mwc-libp2p-noise`, this version
must not be skipped!

- Bump `mwc-libp2p-noise` dependency to `0.21`.
...
- New `mwc-libp2p-request-response` crate
...
- `mwc-libp2p-core-derive`: Fully qualified std::result::Result in macro
...
- `mwc-libp2p-core`: `PeerId`s that use the identity hashing will now be properly
...
- `mwc-libp2p-core`: Updated to multihash 0.11.0.
...
- `mwc-libp2p-noise`: Added the `X25519Spec` protocol suite which uses
  mwc-libp2p-noise-spec compliant signatures on static keys as well as the
  `/noise` protocol upgrade, hence providing a mwc-libp2p-noise-spec compliant
  ...
  included in the mwc-libp2p-noise-spec. The `X25519Spec` implementation
...
- `mwc-libp2p-kad`: Consider fixed (K_VALUE) amount of peers at closest query
...
- `mwc-libp2p-kad`: Provide more insight into, and control of, the execution of
...
- `mwc-libp2p-core`: Treat connection limit errors as pending connection errors.
  [PR 1546](https://github.com/libp2p/rust-libp2p/pull/1546)

- `mwc-libp2p-core-derive`: Disambiguate calls to `NetworkBehaviour::inject_event`.
  [PR 1543](https://github.com/libp2p/rust-libp2p/pull/1543)

- `mwc-libp2p-floodsub`: Allow sent messages seen as subscribed.
...
- `mwc-libp2p-core`: Finished "identity hashing" for peer IDs migration.
...
- `mwc-libp2p-kad`: Allow customising the maximum packet size.
  [PR 1502](https://github.com/libp2p/rust-libp2p/pull/1502)
- `mwc-libp2p-kad`: Allow customising the (libp2p) connection keep-alive timeout.
  [PR 1477](https://github.com/libp2p/rust-libp2p/pull/1477)
- `mwc-libp2p-kad`: Avoid storing records that are expired upon receipt (optimisation).
  [PR 1496](https://github.com/libp2p/rust-libp2p/pull/1496)
- `mwc-libp2p-kad`: Fixed potential panic on computing record expiry.
  [PR 1492](https://github.com/libp2p/rust-libp2p/pull/1492)

- `mwc-libp2p-mplex`: Guard against use of underlying `Sink` upon
...
- Modified mwc-libp2p-floodsub to no longer hash the topic. The new behaviour is now compatible with go-libp2p and js-libp2p, but is a breaking change with regards to rust-libp2p.
- Added mwc-libp2p-pnet. It makes it possible to protect networks with a pre-shared key (PSK).
...
- mwc-libp2p-noise now compiles for WASM targets.
...
- Fixed the `mwc-libp2p-noise` handshake not flushing the underlying stream before waiting for a response.
...
- Refactored `mwc-libp2p-identify`. Some items have been renamed.
...
- Fixed potential arithmetic overflows in `mwc-libp2p-kad` and `multistream-select`.
...
- Fixed tasks sometimes not being notified when a network event happens in `mwc-libp2p-mplex`.
- Fixed a memory leak in `mwc-libp2p-kad`.
...
- Major fixes and performance improvements to mwc-libp2p-kad.
- Initial prototype for record storage in mwc-libp2p-kad.
...
- Fixed a vulnerability in ED25519 signatures verification in mwc-libp2p-core.
...
- Removed `Transport::nat_traversal` in favour of a stand-alone `address_translation` function in `mwc-libp2p-core`.
...
- Fixed wrong version of mwc-libp2p-noise.
...
- Added `mwc-libp2p-noise` that supports Noise handshakes, as an alternative to `libp2p-secio`.
